...
async function processPPN(fileName, fileUrl) {
  await new Promise((resolve, reject) => {
    ...
    .then((file) => {
      console.log(`in then`);
      console.log(file);
      return fs.unlink(`./speech/wake_word/${file}`, function(err) {
        if (err) {
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // TODO: This was done in a rush, it needs SO MUCH refactoring
    if (interaction.data.options[0].name === `check`) {
      fs.readdir(`./speech/wake_word`, function(err, files) {
        if (err) {
          console.error(err);
          return reply(interaction, { type: 4, content: `There was an error while trying to find wake word directory.`, flags: 1 << 6 });
        }
        const ppnFile = files.filter(file => path.extname(file) === `.ppn`);
      ...
      reply(interaction, { type: 4, content: `I answered your command as a private message.`, flags: 1 << 6 });

      await Bot.users.fetch(interaction.member.user.id)
        .then((user) => {
          return user.send(`Please reply with the wake word .ppn file attached. (Interaction will time out in 2 minutes). Reply with \`cancel\` to cancel the process.`)
            .then((message) => {
              ...
              collector.on(`collect`, async (msg) => {
                ...
                if (msg.content.toLowerCase() === `cancel`) {
                  return msg.reply(`Wake word update process cancelled by user!`)
                    .catch(console.error)
                    .finally(() => {
                  ...
                  if (ppnFiles.size > 1) {
                    return msg.reply(`Multiple wake word .ppn files detected, please upload the latest one only!`);
                  }
                  else if (ppnFiles.size === 0) {
                    return msg.reply(`You did not supply a valid wake word .ppn file!`);
                  }
                  else {
                    ...
                    if (daysLeft < 0) {
                      return msg.reply(`The wake word you uploaded has expired already or expires today!`);
                    }
                    else {
              ...
              return collector.on(`end`, (msg, reason) => {
                return message.reply(`Wake word update process timed out!`)
                  .catch(console.error)
                  .finally(() => {
                    collector.removeAllListeners();
                    collector.stop();
